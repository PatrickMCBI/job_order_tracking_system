<?php
session_start();
if(!isset($_SESSION['user']) || $_SESSION['user']['role']!=='cutting'){
...
<h1>Cutting & Pairing Dashboard</h1>
<h3>Jobs Ready for Cutting & Pairing</h3>
...
<tbody>
<?php
$stmt=$conn->query("SELECT * FROM job_orders WHERE status='In Cutting' ORDER BY created_at DESC");
$counter=1;
while($row=$stmt->fetch_assoc()){
    ...
    <td>{$row['deadline']}</td>
    <td>
      <a href='../jo/update_status.php?id={$row['id']}&status=In Sewing' class='btn btn-sm btn-success'>Done Cutting</a>
    </td>
    </tr>";
    $counter++;
}
