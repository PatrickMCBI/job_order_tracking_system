...
<h1>Heat Press Dashboard</h1>
<h3>Jobs Ready for Heat Pressing</h3>
...
<tbody>
<?php
$stmt=$conn->query("SELECT * FROM job_orders WHERE status='Done Printing' ORDER BY created_at DESC");
$counter=1;
while($row=$stmt->fetch_assoc()){
    ...
    <td>{$row['deadline']}</td>
    <td>
      <a href='../jo/update_status.php?id={$row['id']}&status=In Cutting' class='btn btn-sm btn-success'>Done Heat Press</a>
    </td>
    </tr>";
    $counter++;
}
