...
<h1>QC & Packing Dashboard</h1>
<h3>Jobs Ready for QC & Packing</h3>
...
<tbody>
<?php
$stmt=$conn->query("SELECT * FROM job_orders WHERE status='In QC' ORDER BY created_at DESC");
$counter=1;
while($row=$stmt->fetch_assoc()){
    ...
    <td>{$row['deadline']}</td>
    <td>
      <a href='../jo/update_status.php?id={$row['id']}&status=Ready for Sales' class='btn btn-sm btn-success'>QC Done</a>
    </td>
    </tr>";
    $counter++;
}
