...
<tbody>
<?php
$stmt=$conn->query("SELECT * FROM job_orders WHERE status='In Sewing' ORDER BY created_at DESC");
$counter=1;
while($row=$stmt->fetch_assoc()){
    ...
    <td>{$row['deadline']}</td>
    <td>
      <a href='../jo/update_status.php?id={$row['id']}&status=In QC' class='btn btn-sm btn-success'>Done Sewing</a>
    </td>
    </tr>";
    $counter++;
}
